...
export class GameAnimations {
    ...
    performSeamlessSlide(animatedCard, animationArea, targetElement, card, playerId, gameInstance, isMobile) {
        ...
        if (isMobile) {
            // Mobile: 60fps optimized easing with GPU acceleration - visible but fast
            animatedCard.style.transition = 'transform 0.5s cubic-bezier(0.25, 0.46, 0.45, 0.94)';
            animatedCard.style.transform = `translate3d(${deltaX}px, ${deltaY}px, 0) scale(0.8) rotate(1deg)`;
        } else {
            // Desktop animation - visible but fast
            animatedCard.style.transition = 'transform 0.4s cubic-bezier(0.4, 0.0, 0.2, 1)';
